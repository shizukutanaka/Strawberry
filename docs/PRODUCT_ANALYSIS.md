...
| 23 | 不当係争のアカウンタビリティ（自己批判ソクラテス問答で発見） | 高（#21 が導入した非対称: 係争は active 注文を凍結しプロバイダの完了・支払・評判をブロックするが、棄却されても申請者は無償＝連続グリーフィング/DoS が可能だった） | ✅ 実装済み（uphold 裁定で申請者の `deniedDisputeCount` を加算、閾値超過（既定3, `MAX_DENIED_DISPUTES`）で新規係争を 403 ブロック） |
...
- **結論: #21 はプロバイダのアカウンタビリティを実装したが片側だけ — 私自身が借り手のグリーフィング/DoS 経路を作っていた。** 原則: 解決済み係争がプロバイダのコストになるなら、棄却された係争は申請者のコストにならねばならない。是正: uphold で `dispute.raisedBy` の `deniedDisputeCount` を加算し、閾値（既定3）超過で新規係争を 403 ブロック。プロバイダの slash と対称な、申請者側の自己制限を与える。
...
土台（認証・認可・監査・原子的永続化・状態機械・テスト）は堅牢になった。一方でプロダクトの看板である「P2P」と「Lightning 実決済」は外部依存が未接続のため、現状の実態は**単一ノードの GPU 貸出 REST API + 決済抽象層**である。不足機能 28 件中 26 件が実装済み（291/293 テスト通過、2 件は外部インフラ依存でスキップ）。次の価値順は (1) UI または API クライアント、(2) regtest LND での決済 E2E、(3) DB 移行。

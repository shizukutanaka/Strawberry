...
| 11 | GPU 時間帯予約（スケジュール貸出・カレンダー） | 中（現状は「今すぐ借りる」のみ） | 🟡 API のみ（`scheduledStartAt` 指定 + 時間帯重複チェック）。UI は「今すぐ借りる」だけで、カレンダー照会 `GET /gpus/:id/schedule`・手動ブロック `POST /gpus/:id/block` は画面から一度も呼ばれなかったため 2026-09 に削除 |

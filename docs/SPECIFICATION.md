...
2. **ドメイン層の配線は実装済**: `src/marketplace/marketplace-service.js` が
   価格→エスクロー→決済→検証→解放→レピュテーション更新を1本のフローに合成
   （openOrderEscrow / recordPaid / verifyAndSettle / resolveDispute / quoteGpu / rankCandidates）。
   **残るは HTTP ルートハンドラからの呼び出し（薄いラッパ）と LN/実ジョブ収集の結線**。← 次の山

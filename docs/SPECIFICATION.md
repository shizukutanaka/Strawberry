...
| GET/POST `/api/v1/orders` … `/:id/start` | 注文 | JWT | ✅(create スキーマ不整合/param検証/状態遷移バグ修正済, 統合テスト有, 参照価格記録) |
...
2. 価格: 現状 `pricePerHour/12` のフラット … 🟡 **特徴量価格は advisory 配線済**（注文へ `referencePrice`/`priceDeviationPct` を記録、実請求は不変）

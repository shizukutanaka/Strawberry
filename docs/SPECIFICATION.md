...
| **Provider reputation** | stake, slashCount, sla, auditPass/Fail | （無し） | ❌ |
| **Escrow** | orderId, invoice, preimageHash, state, deadline | （無し） | ❌ |
| **Verification record** | jobId, audited, outputs, consensus | （無し） | ❌ |
...
### F2. 信頼基盤（最優先トリオ）
- **計算検証 Proof-of-Compute**: ❌→🟡 土台 `src/verification/work-verifier.js` 実装済（再実行監査/ternary consensus/ゼロ負荷検出）。**未配線**。
- **Lightning エスクロー**: ❌→🟡 本書で `src/payments/escrow-state-machine.js` 実装（hold invoice ライフサイクル）。**LN実機連携・配線は未**。
- **GPU アテステーション**: ❌（nvtrust 連携未, カテゴリ3）。

### F3. レピュテーション/インセンティブ
- ステーク/スラッシング/レピュテーション: 🟡 土台 `src/reputation/reputation-scorer.js` 実装済。**ストレージ・配線未**（Provider reputation エンティティ ❌）。
...
1. **エスクロー状態機械**（本コミットで実装）— hold invoice の held→settle/cancel/dispute を純 FSM 化。`work-verifier` の検証結果で解放判断。
2. work-verifier / reputation-scorer / feature-pricer の**配線**（マッチング・決済解放・出品価格）。
3. Provider reputation / Escrow / Verification record の**永続化エンティティ**追加（当面 JSON, 将来 Prisma）。
...
- `src/payments/escrow-state-machine.js` — エスクロー FSM（本コミット）

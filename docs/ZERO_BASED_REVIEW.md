...
### 第178ラウンド（ソクラテス式問答 — 「公開統計の情報露出は適切か？」）

- **問い**: 認証不要の `GET /marketplace/stats` と係争解決の providerId
  受入 — 財務情報の漏洩・第三者 provider への精算誘導は防げているか。
- **答え**: **全て強制済み（修正対象ゼロ）** —
  - `/marketplace/stats`: `topGpus` から累積収益 `totalSats` を意図的に除外
    （「非公開財務情報 — 未認証レスポンスから除外」の明記あり）、
    応答は件数・価格レンジ・ベンダー分布・完了数の集計のみで
    ユーザー個別データなし。
  - 係争解決の providerId 受入は order.providerId との一致を検証し、
    不一致を 400 で拒否 — admin が任意 providerId を指定して無関係
    プロバイダ宛の精算を起動する誘導を防止（declare-and-enforce の模範形）。

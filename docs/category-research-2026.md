...
- [x] 9. 分散学習・フェデレーテッド・機密性（DiLoCo / secure aggregation / DP）
- [x] 10. 運用・可観測性・データ層・持続可能性（observability / DB / carbon / audit anchoring）
...
**Strawberry の現状**: **単一 GPU 貸出のみ**。複数プロバイダの GPU を束ねた分散学習のオーケストレーションが無く
（`p2p-network.js` は無効）、借り手のコード/データは host から丸見え（カテゴリ3/16 と関連）。

### 参照（arXiv / GitHub）
1. arXiv:2412.01152 — INTELLECT-1 / PRIME（ElasticDeviceMesh、DiLoCo＋int8 で帯域 400× 削減）— https://arxiv.org/html/2412.01152v1
2. arXiv:2407.07852 — OpenDiLoCo: Open-Source Globally Distributed Low-Communication Training（2大陸越しで 90–95% 利用率）— https://arxiv.org/pdf/2407.07852
3. arXiv:2506.21263 — DiLoCoX: Low-Communication Large-Scale Training for Decentralized Cluster — https://arxiv.org/html/2506.21263v1
4. arXiv:2503.11023 — Beyond A Single AI Cluster: Survey of Decentralized LLM Training — https://arxiv.org/html/2503.11023v1
5. arXiv:2410.11368 — Secure Stateful Aggregation（DP-FL）— https://arxiv.org/html/2410.11368v1
6. arXiv:2604.07125 — DDP-SA: Distributed DP ＋ Secure Aggregation — https://arxiv.org/pdf/2604.07125
7. GitHub: learning-at-home/hivemind — インターネット越し分散学習（DHT、BitTorrent 風）— https://github.com/learning-at-home/hivemind
8. GitHub: bigscience-workshop/petals — 100B+ の分散推論/微調整（Hivemind 上、swarm parallelism）— https://github.com/bigscience-workshop/petals
9. GitHub: PrimeIntellect-ai/OpenDiLoCo — OpenDiLoCo 実装 — https://github.com/PrimeIntellect-ai/OpenDiLoCo
10. GitHub: adap/flower（FL フレームワーク）／OpenMined/PySyft（secure aggregation/DP）／NVIDIA/NVFlare — https://github.com/adap/flower ・ https://github.com/OpenMined/PySyft ・ https://github.com/NVIDIA/NVFlare

### 改善点
- (長期) GPU プールを **Hivemind/OpenDiLoCo の低通信分散学習**の実行基盤として位置づけ（カテゴリ7 中断耐性が前提）。ジョブ定義に「マルチノード分散学習」型を追加し、ElasticDeviceMesh 風の参加/離脱を許容。
- (中期) **secure aggregation（PySyft/NVFlare）＋差分プライバシー**で host が個別勾配/データを復元できないようにする（カテゴリ3 TEE と多層）。
- (中期) **Petals 風の分散推論**もカテゴリ8 と統合して提供（大規模モデルを複数プロバイダで分担）。

---

## 10. 運用・可観測性・データ層・持続可能性（observability / DB / carbon / audit anchoring）

**Strawberry の現状**: `prom-client` の `/metrics` はあるが、分散トレーシング（OTel）・外部監査アンカー・
カーボン指標が無い。`src/utils/anomaly-detector.js`・`src/api/middleware/audit.js` はあるが、データ層は
`src/db/json/*`（**並行書込み保護/トランザクション無し**）で Prisma/pg/knex は未配線（三重化）。

### 参照（arXiv / GitHub）
1. arXiv:2505.23554 — Carbon-Aware and Water-Efficient LLM Scheduling（SLIT）— https://arxiv.org/abs/2505.23554
2. arXiv:2304.07948 — Sustainable AIGC Workload Scheduling（Multi-Agent RL）— https://arxiv.org/abs/2304.07948
3. arXiv:2501.15504 — Task Scheduling in Geo-Distributed Computing: A Survey — https://arxiv.org/pdf/2501.15504
4. arXiv:2310.09193 — Tikuna: P2P Network Security Monitoring（cross-ref カテゴリ6）— https://arxiv.org/pdf/2310.09193
5. GitHub: open-telemetry/opentelemetry-collector — トレース/メトリクス/ログ統合 — https://github.com/open-telemetry/opentelemetry-collector
6. GitHub: prometheus/prometheus ／ grafana/grafana — 監視・可視化 — https://github.com/prometheus/prometheus ・ https://github.com/grafana/grafana
7. GitHub: opentimestamps/opentimestamps-client — BTC ブロックチェーン・タイムスタンプ（監査ログ非否認）— https://github.com/opentimestamps/opentimestamps-client
8. GitHub: Green-Software-Foundation/carbon-aware-sdk — 系統カーボン強度 API（Prometheus exporter/Grafana）— https://github.com/Green-Software-Foundation/carbon-aware-sdk
9. GitHub: thegreenwebfoundation/co2.js — 排出推定 npm モジュール — https://github.com/thegreenwebfoundation/co2.js
10. GitHub: prisma/prisma — データ層統一先（pg/knex 整理）— https://github.com/prisma/prisma

### 改善点
- (短期) `/metrics` に **OpenTelemetry トレース**を追加し、Grafana/Loki/Tempo で可観測性を統合（`service-monitor.js`/`logger.js` と結線）。
- (短期) **監査ログ（`audit.js`）の定期 Merkle ダイジェストを OpenTimestamps で BTC アンカー**し非否認性を確立（§18、BTC を既に扱うため親和的）。
- (中期) **carbon-aware-sdk / co2.js** でカーボン強度を取得し、配置スコア（カテゴリ4/15）と「グリーン実行」表示に活用。
- (中期) **データ層を Prisma/Postgres へ統一**し、JSON の並行書込み問題・整合性制約（注文/決済/残高）を解消（`ARCHITECTURE.md` follow-up）。
- (中期) `anomaly-detector.js` を **Tikuna 流の P2P 層攻撃監視**へ拡張（カテゴリ6）。

---

## 総括（10カテゴリ × 約10参照 完了）

10カテゴリ全てに arXiv 論文＋GitHub リポジトリ等を集約し、各々をコードへ紐づけて改善点を洗い出した。
**最優先トリオ**は横断分析（`docs/improvement-research-2026.md`）と一致:

1. **計算検証 Proof-of-Compute**（カテゴリ1）— 現状ゼロ。ランダム再実行監査＋GPU profiling＋楽観的検証（PoL 単体に依存しない）。
2. **Lightning エスクロー**（カテゴリ2）— hold invoice/HTLC で前払いロック＋条件付き解放。`payment.js` の非アトミック送金の根本対策。
3. **GPU アテステーション**（カテゴリ3）— `nvtrust` で真正性検証し、`nvidia-smi` 自己申告のスペック詐称を排除。

これらを支える**信頼基盤**: レピュテーション/ステーク・スラッシング（5）、P2P 堅牢化（6）。
**製品競争力**: 価格/オークション（4）、中断耐性 spot（7）、推論サービング効率（8）、分散学習（9）。

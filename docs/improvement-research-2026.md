...
1. (中期) プロバイダに **担保ステーク**を要求し、§1 の検証不一致・SLA 違反で **slashing**。
   → **実装済（部分）**: ステーク台帳 API（POST/GET /marketplace/stake）、スラッシュでの担保没収、
   出品ゲート（MIN_PROVIDER_STAKE_SATS）、および **アンボンディング出金**（EigenLayer/Cosmos 型:
   出金は「申請→STAKE_UNBOND_MS（既定72h）猶予→claim」の2段で、猶予中も担保はスラッシュ対象のまま残り
   hit-and-run を防止。猶予中のスラッシュで目減りした分は出金できない）。残: LN ロック実装。

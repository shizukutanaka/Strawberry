...
- SPA フォールバック修正: `app.get('*')` が API 系パス（apiPrefix・/api/・/graphql・/master-auth・/metrics 等）の未知ルートにも index.html を 200 で返していた問題を修正。API クライアントは JSON を期待するため、これらは notFoundMiddleware の JSON 404 に落とす。

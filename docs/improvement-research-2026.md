...
- アカウント退会（DELETE /users/me）に再認証を追加: JWT 所持のみで PII 匿名化＋全セッション失効という不可逆操作が実行できたため、OWASP「sensitive operations require re-authentication」に倣い、パスワード照合（bcrypt）を必須化。パスワードを持たない OAuth 専用アカウントは登録メール再入力で代替。`authLimiter` も付与して確認パスワードの総当たりを防止。

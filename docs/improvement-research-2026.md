...
### その他実装済（死活監視の通知スパム抑止）
- `src/p2p-notify.js` が 15 秒ポーリングで対象が落ち続ける間、毎回全チャネルへ通知していた（アラートストーム）。対象ごとの状態遷移時のみ通知に変更（up→down で1回・down→up で復帰1回）。既定監視対象も admin 専用 `/api/system/info`（認証なしでは常時401で誤発報）から公開 liveness `/health` へ修正。`p2p-health` 依存を遅延化し libp2p 未導入環境でもモジュール読み込み可能に。
||||||| 5c3f4ed

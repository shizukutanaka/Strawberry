...
1. (中期) §4 のオークションに **Shill Score 風の異常検知**（`src/utils/anomaly-detector.js` を拡張）を組み込み、§5 のステーク・スラッシングで動的ペナルティ。
   → ✅ **実装済**（`src/marketplace/shill-detector.js`：慢性敗北者・カバー入札・専属共演・ローテーション談合ペアの4シグナルを重み合成してスコア化、つり上げ受益者も併記。入札履歴は `BidRepository`/`bids.json` に永続化され `selectProvider` がオークション毎に検知→監査ログ `auction_collusion_suspected`＋`reportAnomaly` 連携、`GET /api/v1/marketplace/auction/suspicions` で照会。デフォルトは報告のみ、オプトインでフラグ済み入札除外（`excludeFlagged`）・レピュテーションスラッシング（`enforceShillSlash`））
...
| # | 改善領域 | 優先度 | 根拠（代表） |
|---|---------|--------|-------------|
| 17 | オークション談合/シル入札検知 | ~~中（§4の対）~~ ✅実装済 | arXiv:1812.10868, 2506.00282 |

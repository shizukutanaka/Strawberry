...
### 実装状況（2026-08）— アクション2（GPU profiling によるゼロ負荷検出）を実レンタルへ結線

着手前の実態: `work-verifier.detectZeroLoad()` は実装済みだったが、入力の `utilSamples` は
**admin が `/escrow/:id/verify` のリクエストボディで手渡す経路しか無く、実レンタルからは
一度も呼ばれていなかった**。つまり「借りた GPU が本当に計算したか」の検証は、部品はあっても
稼働していなかった。

実装（`src/verification/utilization-collector.js`）:

- ハートビートに任意フィールド `utilizationPct` を相乗りさせてサンプルを収集し、注文終了時
  （`/stop`）に判定して `order.utilizationAudit` として保存する。
- **片側の自己申告は証拠にならない**、が設計の中心。プロバイダのホストは実測値を持つが
  検出したい不正の当事者なので「常に 90%」と答えれば済む。借り手は返金目的で「ずっと遊休
  だった」と偽る動機がある。したがって**両者から集めて食い違いを検出する**:
  - 両者が遊休で一致 → `zero_load`（偽る動機が逆向きの 2 者が同じことを言っている）
  - 食い違う → `disputed`（どちらかが偽っている。どちらかは断定しない）
  - サンプル不足 → `insufficient`、テレメトリ皆無 → 保存しない（`no_data` を保存すると
    「検証した結果シロ」と誤読される）
  §1 の ternary consensus を、その場にいる 2 者へ適用した最小形にあたる。
- **資金は自動で動かさない**。判定は証拠であって裁定ではないので、`zero_load` でも注文は
  完了し精算は通常どおり進む。レピュテーションの `auditPasses`/`auditFails` に反映し、
  監査ログに残し、UI で双方に開示して係争の材料にするに留める（自動スラッシュは §5 の範疇）。
  `disputed` は**どちらのカウンタにも数えない** — 食い違いは「プロバイダが不正」を意味しない。
- サンプルはリングバッファで注文あたり 500 件に制限し、終了時に破棄する。

**これは暗号学的証明ではない**。TEE アテステーション（§2）や再実行監査の代替ではなく、
「何もせず課金」という最も素朴な不正を追加ハードウェア無しで検出可能にし、双方の申告を
突き合わせ可能な監査証跡として残すためのもの。アクション1（ランダム再実行監査）と
アクション3（ZK/TEE）は引き続き未着手。
...
### 実装状況（2026-08）— 1 と 3 は実装済、2 は未着手

着手前の実態: `feature-pricer.js` に `spotIdleDiscount` / `market.spotIdle` があったが、
**どこからも渡されていない完全な死にコード**だった（grep 済）。注文は固定時間枠の専有レンタル
のみで、中断の概念そのものが存在しなかった。

実装したもの（`src/marketplace/spot-tier.js` = 純関数のポリシー層）:

- **中断許容ティア**: 出品側が `spotEnabled` でオプトインし、`spotDiscountPct`（10〜90%）で
  割引率を決める。借り手は注文時に `tier: 'spot'` を選ぶ。オプトインしていない GPU への
  spot 指定は 400 で弾く（黙って定価にすると借り手は中断リスクだけ負って割引を得られない）。
- **猶予窓つきの中断**: `POST /orders/:id/preempt`（プロバイダ/admin）。即時停止ではなく
  `preempting` 状態へ移し、既定 120 秒（30〜900 秒）の猶予後に停止・精算する。猶予を一級の
  概念にしたのは Bamboo (arXiv:2204.12013) が「素朴なチェックポイントだと再起動に 77% の時間を
  浪費する」＝猶予ゼロの中断はティア自体を使い物にならなくすると示しているため。
  借り手は保存が済み次第 `/stop` で自分で確定できる。
- **中断率の開示**: 注文履歴から導出（新規の永続化は増やさない）。GPU 詳細と rent モーダルに
  表示する。サンプル不足時は「0%」と偽らず「蓄積されていない」と表示する。

**設計上いちばん重要な点（zero-work theft の再発防止）**: `/stop` がプロバイダに禁止されて
いるのは、「accept → 借り手が支払い → 借り手が start → プロバイダが即 stop」で 0 秒の労働に
対して全額を回収できてしまうため。spot はプロバイダによる打ち切りを仕様として認めるので、
この穴を別の形で塞ぐ必要がある:
1. spot 注文限定（専有注文は従来どおり打ち切れない）
2. 猶予窓（即時停止させない）
3. **精算で最低課金 floor を効かせない**。`settlement-calculator` の既定は「即時解約でも総額の
   10% は課金」だが、これは*借り手都合*の解約を想定した floor である。プロバイダ都合の中断に
   適用すると「受注 → 即中断 → 最低課金だけ回収」が成立してしまうため、中断時は
   `minChargeRatio: 0` で厳密な従量按分にする。

中断は SLA 違反として reputation を減点しない（ティア仕様どおりの動作を罰することになる）
代わりに、中断率を必ず開示する。「安いが中断される」ことと「約束を破る」ことは別で、
前者を隠すと spot は信頼できない商品になる。

**未着手**: チェックポイント退避（TierCheck arXiv:2605.17821 の三層）と別プロバイダへの
再スケジュール。これらは実コンテナ制御（Docker/k8s 実機）が前提で、猶予窓という土台が
無いと設計できなかった。今回その土台ができたので、次の増分として着手可能。
...
- **2 = 実装済**: `src/reputation/provider-uptime.js` がハートビートから稼働安定度スコア／tier を算出し、
  `GET /gpus`・`/gpus/:id` の `reliability` と `?sort=reliability` に反映済み。サンプル不足は「計測中」として
  断定を避ける。
- **1 = 実装済**: `src/gpu/perf-score.js`（本増分）。ただし**実機ベンチの必須化は行っていない** —
  実測ベンチはプロバイダの自己申告になり、そのまま順位に効かせると検索順位を金で買えてしまうため、
  スコアは「検証可能な構造スペック（型番・VRAM・電力）＋参照表」から導き、自己申告値は
  *上限を課したうえで* しか効かせない設計にした。具体的には:
  - 型番を参照表（既知 GPU の公開スペック）と照合し、演算・帯域・VRAM の**加重幾何平均**で
    参照GPU(RTX 4090 級)=100 の相対指数を出す（Roofline: 実効性能は演算とメモリ帯域の両方に支配される）。
  - **申告 VRAM が型番と矛盾する出品は参照表を採用しない**（"H100" を名乗る 24GB の出品に H100 の
    スコアを与えない）。矛盾は `findings` として GPU 詳細ページに開示する。
  - 申告 TFLOPS は **消費電力由来の物理上限**（2.5 TFLOPS/W）でクランプ。
  - **未検証の未知型番は参照GPU 超えを認めない**（アテステーション提出で解除）。
  - 根拠が無ければスコアを推測せず `null`（UI は「未算出」）。
  - 価格対性能 `perfPerHourSat`（DLPerf/$ 相当）と `?sort=perf|value` を追加。
  残る課題: 参照表に無い新型 GPU は上限に張り付くため、表のメンテナンスか実機アテステーションの
  普及が前提になる。§1 の実行時 GPU profiling と突き合わせれば、申告ベースのスコアを
  実測で裏取りできる（未着手）。
...
### 実装状況（2026-08）— 可視化と検索順位への反映まで実装

着手前の実態: 出品には `location { country, region, city, latitude, longitude }` があったが、
**国フィルタ以外に一切使われていなかった**（`carbon` / `CO2` を含むコードは皆無）。

実装（`src/gpu/carbon-intensity.js`）:

- 申告所在地から系統カーボン強度（gCO2eq/kWh）を引き、PUE を掛けて 1 時間あたりの推定
  排出量を算出。`GET /gpus`・`/gpus/:id` の `carbon` と `?sort=carbon`（低炭素順）で公開。
- **国内差の大きい地域はサブリージョンを持つ**。米国の州は 90〜750 と 8 倍以上開くため、
  国単位で丸めるとワシントン州の水力とウェストバージニア州の石炭が同点になる。
- 関連ソフトウェア: Google の carbon-intelligent computing（系統強度に応じてバッチを
  時間・地域シフト）、ElectricityMaps / WattTime（地域別リアルタイム強度 API）。
  後者を差し込めるよう `setIntensityProvider()` を用意した（既定は静的表）。

**この機能の失敗モードはグリーンウォッシングなので、以下を明示的に設計に組み込んだ**:

1. **所在地は自己申告で未検証**。実際にはポーランドで動かしながら「ノルウェー」と申告できる。
   出力には常に `confidence: 'self-declared-location'` を付け、UI も「申告地に基づく推定」と
   断る。検証済みの環境認証であるかのようには決して見せない。
2. **不明な地域は推測しない**。世界平均で埋めると、実際には高炭素の地域が中庸に見えてしまう。
   `null` を返し UI は「炭素: 不明」と表示する。
3. **`?sort=carbon` で強度不明を末尾に回す**。0 扱いで最上位に出すと、所在地を申告しない
   出品が最もグリーンに見え、申告を省くのが最も有利になる。
4. 年間平均であってリアルタイムの限界強度ではないことも UI のツールチップに明記した。

**未着手**: §4 のマッチング／オークションへの carbon-aware な重み付け（SLIT arXiv:2505.23554 の
TTFT・カーボン・水・電力コスト共最適化）と、「グリーン実行」のプレミアム価格付け。
現状は借り手が自分で低炭素を選べるようにしたところまで。
...
### 実装状況（2026-08）— 実装済（限界つき）

着手前の実態は、`docs/SPECIFICATION.md` の記述（「残るは OTS への root 実提出のみ」）より
悪かった。`merkle-anchor.js` / `audit-anchor.js` は純関数として完成していたが、
**`anchorAuditLogFile()` は `src/` のどこからも呼ばれておらず（テストのみ）、定期ジョブが
無いためアンカーが 1 つも生成されていなかった**。アンカーを外部へ公開する経路も無く、
root が運営のディスクから出ない以上、本節が名指しする脅威は全く緩和されていなかった。

実装したもの:

- **定期増分アンカリング** — `src/security/anchor-scheduler.js`（`invoice-poller` と同型:
  `setInterval` + 再入ロック + `unref`）。前回アンカーの `toByteOffset` から**末尾だけを読む**
  （旧実装は毎回ログ全体を `fromIndex=0` で再アンカーしており、範囲の重複したアンカーが
  増え続け、上限 50MB のログを毎サイクル全パースしていた）。
- **OTS 提出** — `src/security/ots-client.js`。`POST {calendar}/digest` に生 32 バイトを送る。
  **複数カレンダーへ冗長提出**するのは本質的要件（単一カレンダー運営者が Strawberry 運営者と
  結託すれば外部アンカリングの意味が消える）。既定無効・fail-soft・SSRF ガード経由。
- **commitment の公開** — `GET /api/v1/audit-anchors/latest` と
  `GET /api/v1/audit-anchors/{root}/receipt`（いずれも認証不要）。第三者が commitment を
  保持していなければ後日の改ざんは誰にも検出できないので、公開は飾りではなく本体。
  エントリ本体と包含証明は監査ログの内容を露出するため admin 限定（`/admin/audit-anchors*`）。
- ついでに直した実バグ 2 件:
  - `audit-anchor.js` がモジュール読込時にログパスを固定しており、`audit-log.js` が
    呼び出しごとに `AUDIT_LOG_PATH` を解決するのと食い違っていた（同変数を設定した環境で
    アンカラーが別ファイルを見る）。
  - `src/api/middleware/audit.js` が「audit.log とは別ファイルにすること」と自らコメントで
    警告しながら、移動先の指定に**同じ `AUDIT_LOG_PATH`** を使っていた。設定した瞬間に
    両者が同一ファイルへ書き込み、コメントが警告するチェーン破壊が再発する状態だった
    （`ACCESS_AUDIT_LOG_PATH` に分離）。

**意図的にやっていないこと（overclaim を避ける）**:

1. **OTS バイナリ証明の自前パース／Bitcoin アテステーションの自前検証はしない**。
   カレンダーの返却バイト列を base64 の不透明レシートとして保存するだけ。検証を自前実装して
   「Bitcoin タイムスタンプ検証済み」と称するのは誇張になる。レシートを verbatim で保持すれば
   第三者が標準 `ots verify` / `ots upgrade` で自分で検証でき、**検証主体を運営から第三者へ
   移す**という本機能の主旨にもむしろ合致する。Bitcoin 確定待ちのアップグレード取得も
   同じ理由で対象外（commitment 算出に提出レスポンスのパースが要る）。
2. **連続するアンカー間の consistency proof が無い**（RFC 6962 Certificate Transparency との差）。
   本設計は範囲ごとに独立した Merkle 木を作るため、運営がアンカー N と N+1 の**間**でログを
   書き換えた場合、古い root を保持している第三者だけが検出できる。CT は成長する単一木と
   consistency proof でこれを構造的に防ぐ。緩和として (a) アンカーに `prevAnchorRoot` を持たせて
   アンカー列を連鎖させ、(b) 既存の HMAC 連鎖が範囲境界をまたいでエントリを結ぶ、の 2 段を
   置いているが、**CT と同等ではない**。
3. **アンカー間隔内の未アンカー・エントリは従来どおり HMAC 連鎖だけが頼り**（既定 1 時間。
   間隔を縮めれば窓は狭まる）。
4. **50MB 上限でエントリが捨てられる問題は解決しない**（`audit-log.js` はローテートせず破棄する）。
   書かれなかったエントリはアンカーのしようがない。

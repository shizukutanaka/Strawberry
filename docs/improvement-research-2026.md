...
### その他実装済（実行時生成物の書き込み先）
- `src/p2p-health.js` / `src/p2p-gpu.js` が `src/` 配下へ JSON を書いていたため、非 root コンテナ（Dockerfile.api 非 root 化）やパッケージ化で書き込み不可になるのを `data/` 配下へ移行（`P2P_HEALTH_FILE` / `P2P_GPUS_FILE` で上書き可）。`atomicWriteJSON` が親ディレクトリを自動生成するため既存 `data/` gitignore もそのまま有効。

...
- docker/Dockerfile.api を非 root 実行化（CIS Docker Benchmark 4.1）。node 公式同梱の node ユーザーで起動し、実行時書き込み先（data/・backups/・logs/）のみ node 所有、アプリ本体は root 所有のまま読み取り専用相当に。書き込み可能領域を最小化。

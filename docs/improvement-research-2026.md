...
1. (中期) プロバイダ・メタデータに地域/電力カーボン強度を持たせ、§4 のマッチングに**carbon-aware な配置スコア**を追加（遅延非依存ジョブは低炭素地域へ）。
   → ✅ **実装済**（`auction-engine.js` に carbon コンポーネント追加：`weights.carbon`（既定0=オプトイン）と `maxCarbonIntensity` ハード制約、比正規化スコア、未開示は世界系統平均 480 gCO2eq/kWh の仮定値でやや悲観的に採点して開示インセンティブを付与。入札は `carbonIntensity` を自己申告、`ranked` 行に `green`/`carbonIntensity` を注記。`POST /marketplace/auction` に借り手側 opts のホワイトリスト経路を新設）
2. 「グリーン実行」をプレミアム属性として価格・検索に露出。
   → ✅ **実装済**（GPU 登録/更新スキーマの `location.carbonIntensity`、`GET /gpus?green=true`/`?maxCarbonIntensity=` フィルタ、`?sort=carbon` ソート、一覧の `green` フラグ。未開示はフィルタ除外・ソート末尾）
...
| 15 | カーボン対応・地理分散配置 | ~~中~~ ✅実装済 | arXiv:2505.23554, 2304.07948, 2501.15504 |

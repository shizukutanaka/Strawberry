...
> ## ⚠️ 実態レビュー注記（2026-06 / deep research）
>
> 本チェックリスト下部の「[x] 実装済み」項目（約100件）および
> 「カテゴリ別進捗サマリー（XX/XX件 実装済み）」は**実態と乖離**しており、
> 証跡のない自動生成記述を含む（壊れたテンプレート断片 `{{ ... }}` も残存していた）。
> 鵜呑みにせず、必ずコード・テストで裏取りすること。
>
> ### 2026-06 時点で実際に確認・修正した事項（このブランチ）
> - **起動不能を解消**: 未宣言だった依存(express/jsonwebtoken/joi/winston 等)を `package.json` に追加、
>   `server.js` の起動時クラッシュ(`masterAuth.router` 参照ミス / TDZ)を修正、
>   `main` を実在エントリ(`src/api/server.js`)へ修正。`npm install` → 起動が可能に。
> - **重大セキュリティ修正**: ハードコードされた秘密鍵フォールバック(JWT/AES/APIキー/セッション)を廃止し
>   `requireSecret()` で本番fail-fast化、利益受取アドレスAPIに admin 認証を追加、
>   Lightning 送金失敗時の `dummy-txid` 成功偽装を廃止、virtual-gpu-manager のコマンドインジェクションを修正。

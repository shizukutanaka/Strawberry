...
router.get('/admin/pending',
  ...
  asyncHandler(async (req, res) => {
    ...
    // pending 0 件では Map 構築自体が不要な全量スキャンになるため遅延生成する。
    let orderById = null;
    let userById = null;
    const lookupOrder = (id) => {
      if (!orderById) orderById = new Map((OrderRepository.getAll() || []).map(o => [o.id, o]));
      return orderById.get(id);
    };
    const lookupUser = (id) => {
      if (!userById) userById = new Map((UserRepository.getAll() || []).map(u => [u.id, u]));
      return userById.get(id);
    };
    const enriched = sorted.map(p => {
      const order = p.orderId ? lookupOrder(p.orderId) : null;
      const renter = p.userId ? lookupUser(p.userId) : null;
      return {
        id: p.id,
        orderId: p.orderId,
        amount: p.amount,

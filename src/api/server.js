...
// 依存脆弱性の定期監査（既定24時間間隔・起動15分後に初回）。
// 同一脆弱性セットの再通知は security-audit 側のフィンガープリントで抑止。
// テスト環境では起動しない（タイマー積み上がり防止、service monitor と同理由）。
if (process.env.NODE_ENV !== 'test') {
  try {
    require('../core/security-audit-poller').start();
  } catch (e) {
    logger.warn(`security-audit-poller: failed to start: ${e.message}`);
  }
}

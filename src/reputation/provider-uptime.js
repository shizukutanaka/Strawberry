...
// { score: number|null, tier: string, beats, gapEvents, sessions, measuring: boolean }
// score は 0..1（切断頻度が低いほど高い）。サンプル不足時は score=null / measuring=true。
function getReliability(providerId) {
  const empty = { score: null, tier: 'unrated', beats: 0, gapEvents: 0, sessions: 0, measuring: false };
  ...
  if (beats < MIN_BEATS_FOR_SCORE) {
    return { score: null, tier: 'measuring', beats, gapEvents, sessions, measuring: true };
  }

  // 切断率 = gapEvents / beats。低いほど安定。スコアは 1 - 切断率。
  const disruptionRate = _clamp01(gapEvents / beats);
  const score = Math.round(_clamp01(1 - disruptionRate) * 100) / 100;
  const tier =
    score >= 0.95 ? 'excellent' :
    score >= 0.85 ? 'good' :
    score >= 0.6 ? 'fair' : 'poor';

  return { score, tier, beats, gapEvents, sessions, measuring: false };
}

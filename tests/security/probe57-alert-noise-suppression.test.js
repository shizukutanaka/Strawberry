...
// tried to load external notify modules with missing deps and logged a
...
describe('notifyExternalAlert: no module-load noise when channels are unconfigured', () => {
  const saved = {};
  beforeEach(() => {
    for (const k of ['SLACK_WEBHOOK_URL', 'LINE_TOKEN']) {
  ...
  it('emits no "モジュール呼び出し失敗" warning when no channel env is set', async () => {
    ...
    // No load-failure warnings (Slack/LINE module require失敗)
